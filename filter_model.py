...
class FilterModel(pl.LightningModule):
    def __init__(self, config: omegaconf.DictConfig) -> None:
        ...
        # self.proj = torch.tensor(matrix@ut.mT@torch.diag(1/d)@v.mT, device='cuda')
    ...
    def training_step(self, batch: typing.Tuple[torch.Tensor,torch.Tensor], batch_idx: int) -> torch.Tensor:
        ...
        # self.delta += torch.sum((torch.fft.rfft(torch.reshape(self.proj@noise.reshape(noise.shape[0],-1,1), (noise.shape[0],1,noise.shape[2],noise.shape[3])), dim=3, norm="forward").abs())**2, dim=0)[0]
